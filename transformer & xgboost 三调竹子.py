...
if __name__ == '__main__':
    ...
    tifset = TIFDataSet(r"E:\城市与区域生态\大熊猫和竹\平武种群动态模拟\三调竹子transformer")
    pre_loader = DataLoader(tifset, batch_size=20, shuffle=False)

    csvset = TimeSeriesDataset(r'E:\城市与区域生态\大熊猫和竹\卧龙种群动态模拟\主食竹分布模拟\样本点wdrvi_dem.csv')
    train_loader = DataLoader(csvset, batch_size=20, shuffle=False)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    model = torch.load(
        r'E:\城市与区域生态\大熊猫和竹\卧龙种群动态模拟\主食竹分布模拟\三调model_inputsize90_nhead10_numlayer2_lstmsize40_20_lr1e-3.pt')
    model.to(device)
    model.eval()
    ...
    df_4train = pd.read_csv(r'E:\城市与区域生态\大熊猫和竹\卧龙种群动态模拟\主食竹分布模拟\样本点wdrvi_dem.csv', header=0, index_col=0,
                            encoding='utf-8')
    ...
    dst_filename = r'E:\城市与区域生态\大熊猫和竹\平武种群动态模拟\三调竹子transformer\平武三调竹子概率_transfomer_lstm_xgb.tif'
