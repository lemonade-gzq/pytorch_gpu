    ...
    for hdf5_file in os.listdir(filepath):
        ...
        for beam in beams:
            ...
            if beam_data.attrs['description'] == 'Full power beam':
                ...
                # 提取其他字段
                crown_lat = np.array(beam_data["lat_lowestmode"],dtype='float64')
                crown_lon = np.array(beam_data["lon_lowestmode"],dtype='float64')
                ...
                # df = pd.concat([df, calculate_shannon_entropy(rh_data_selected)], axis=1)
                ...
                beam_data_df = pd.DataFrame(beam_data_df, columns=['lat', 'lon', 'rh95'])#'cv','skew','evenness'
